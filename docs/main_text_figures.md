...
> the word-frequency plot as Fig. 1. The image files were renamed to match
> (`fig1_raw_counts.png` -> `fig2_raw_counts.png`, and so on up to
> `fig6_bgamma_by_profile.png` -> `fig7_bgamma_by_profile.png`); the ESM
> Qwen3-only top-20 plot is now `esm_top20_l3_gamma_qwen.png`.
...
**File:** `main_text/fig2_raw_counts.png`
...
![Fig. 2](../data/output/figures/main_text/fig2_raw_counts.png)
...
**File:** `main_text/fig3_sigma_posteriors.png`
...
![Fig. 3](../data/output/figures/main_text/fig3_sigma_posteriors.png)
...
**File:** `main_text/fig4_l2_triple_bar.png`
...
![Fig. 4](../data/output/figures/main_text/fig4_l2_triple_bar.png)
...
**File:** `main_text/fig5_top10_qwen_vs_gemma.png`
...
**Manuscript caption:** The ten most-recommended L3 methods for each model, as a share of that model's total recommendations (the union of the two top-ten lists). The value beside each bar is the method's estimated post-2023 trajectory in the literature (γ from the bibliometric Dirichlet-Multinomial model, §3.3): positive values indicate methods gaining share, negative values methods declining relative to their pre-2023 trend.
...
**File:** `main_text/fig6_bpre_bgamma_posteriors.png`
...
**File:** `main_text/fig7_bgamma_by_profile.png`

**Status:** Candidate for demotion to the ESM. The manuscript currently reports
only the pooled b_gamma interval, and the figure shows a four-panel null.
...
| Top 20 L3 methods (Qwen3 only) | `main_text/esm_top20_l3_gamma_qwen.png` | Qwen3-only version; superseded by combined Fig. 5 |
