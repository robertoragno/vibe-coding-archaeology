...
results_text <- gsub("\\{\\{DIVERGENCES\\}\\}",     n_divergences,                             results_text)
results_text <- gsub("\\{\\{RHAT_SIGMA_BETA\\}\\}",  round(rhat_sigma_beta,  4),               results_text)
results_text <- gsub("\\{\\{RHAT_SIGMA_GAMMA\\}\\}", round(rhat_sigma_gamma, 4),               results_text)
results_text <- gsub("\\{\\{ESS_SIGMA_BETA\\}\\}",   round(ess_sigma_beta),                    results_text)
results_text <- gsub("\\{\\{ESS_SIGMA_GAMMA\\}\\}",  round(ess_sigma_gamma),                   results_text)
results_text <- gsub("\\{\\{SIGMA_BETA_MEAN\\}\\}",  round(sigma_beta_mean,  4),               results_text)
results_text <- gsub("\\{\\{SIGMA_BETA_CI\\}\\}",    sigma_beta_ci_str,                        results_text)
results_text <- gsub("\\{\\{SIGMA_GAMMA_MEAN\\}\\}", round(sigma_gamma_mean, 4),               results_text)
results_text <- gsub("\\{\\{SIGMA_GAMMA_CI\\}\\}",   sigma_gamma_ci_str,                       results_text)
results_text <- gsub("\\{\\{RATIO\\}\\}",            round(sigma_gamma_mean / sigma_beta_mean, 3), results_text)
