...
results_text <- gsub("\\{\\{DIVERGENCES_L2\\}\\}",     n_divergences_l2,                                 results_text)
results_text <- gsub("\\{\\{RHAT_SIGMA_BETA_L2\\}\\}",  round(rhat_sigma_beta_l2,  4),                   results_text)
results_text <- gsub("\\{\\{RHAT_SIGMA_GAMMA_L2\\}\\}", round(rhat_sigma_gamma_l2, 4),                   results_text)
results_text <- gsub("\\{\\{ESS_SIGMA_BETA_L2\\}\\}",   round(ess_sigma_beta_l2),                        results_text)
results_text <- gsub("\\{\\{ESS_SIGMA_GAMMA_L2\\}\\}",  round(ess_sigma_gamma_l2),                       results_text)
results_text <- gsub("\\{\\{SIGMA_BETA_MEAN_L2\\}\\}",  round(sigma_beta_mean_l2,  4),                   results_text)
results_text <- gsub("\\{\\{SIGMA_BETA_CI_L2\\}\\}",    sigma_beta_ci_str_l2,                            results_text)
results_text <- gsub("\\{\\{SIGMA_GAMMA_MEAN_L2\\}\\}", round(sigma_gamma_mean_l2, 4),                   results_text)
results_text <- gsub("\\{\\{SIGMA_GAMMA_CI_L2\\}\\}",   sigma_gamma_ci_str_l2,                           results_text)
results_text <- gsub("\\{\\{RATIO_L2\\}\\}",            round(sigma_gamma_mean_l2 / sigma_beta_mean_l2, 3), results_text)
