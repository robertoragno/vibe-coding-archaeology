...
We retrieved archaeology papers published between 2010 and 2025 from Scopus and retained the computational subset — papers whose abstracts describe a quantitative or computational methodology — yielding 7,355 papers for analysis. Each retained paper's methodology was classified into a two-level taxonomy: sub-disciplines (L2) and specific techniques (L3). Classification was performed with Qwen, a large language model, applied consistently to all abstracts, yielding 25 L2 sub-disciplines and 242 L3 techniques. This gives us a record of how the methodological menu of the discipline has changed, year by year, at fine granularity.
...
The model estimates phi from data using a weakly informative lognormal prior. In the primary L2→L3 analysis (25 L2 groups, 242 L3 methods), phi ≈ 1102 [564, 2073], confirming the field is compositionally regular — observed proportions track the structural trend closely year to year. sigma_gamma = 0.106 [0.010, 0.219].

The fit converged cleanly (Rhat ≤ 1.003, ESS (sigma_gamma) = 640, zero divergences). sigma_gamma < sigma_beta (0.106 vs 0.249): the post-LLM reshuffling is smaller in magnitude than the long-run baseline trend.
...
But the literature has not converged. The Inverse Simpson index has risen from ~88 (pre-2023) to ~112 (post-2023), and this diversifying trend continued uninterrupted through the post-LLM period. sigma_gamma = 0.106 [0.010, 0.219] shows weak evidence of post-2023 reshuffling, but its magnitude is smaller than the pre-existing trend (sigma_gamma < sigma_beta, 0.106 vs 0.249), and no individual method shows a credible shift.
